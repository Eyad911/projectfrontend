  ...
  const getData = async () => {
    if (local) {
      const item = await axios.get(
        `https://ecom-911.herokuapp.com/users/email/${local.email}`
      );
  ...
  const editName = async (e) => {
    ...
    if (edit.length > 0) {
      const editFullName = await axios.put(
        `https://ecom-911.herokuapp.com/users/edit/${local.email}`,
        {
          fullName: edit,
        }
      );
      ...
      {account.map((item, i) => {
        return (
          <div key={i} className="formDiv">
            ...
            <button className="btn btn-danger btn-block" onClick={kick}>
              Logout
            </button>
